...
#
# @param  $1 - if this is the first member of the set, call this function to initialize. Init may differ based on what kind of replicaset
...
function configure_replicaset_mode() {
  ...
  if [[ "$($MONGO --host $(hostfqdn) --eval "${comm}" --quiet)" =~ "${MONGODB_REPLICA_NAME:-}" ]]; then
