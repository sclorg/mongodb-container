...
function setUpMbaasDB(){
  ...
  local js_command="db.getSiblingDB('${MONGODB_FHMBAAS_DATABASE}').addUser({user: '${MONGODB_FHMBAAS_USER}', pwd: '${MONGODB_FHMBAAS_PASSWORD}', roles: [ 'readWrite' ]});"
...
function setUpReporting(){
  ...
  local js_command="db.getSiblingDB('${MONGODB_FHREPORTING_DATABASE}').addUser({user: '${MONGODB_FHREPORTING_USER}', pwd: '${MONGODB_FHREPORTING_PASSWORD}', roles: [ 'readWrite' ]});"
  if ! mongo admin ${1:-} --host ${2:-"localhost"} --eval "${js_command}"; then
...
function setUpMetrics(){
  ...
  local js_command="db.getSiblingDB('${MONGODB_FHMETRICS_DATABASE}').addUser({user: '${MONGODB_FHMETRICS_USER}', pwd: '${MONGODB_FHMETRICS_PASSWORD}', roles: [ 'readWrite' ]});"
  if ! mongo admin ${1:-} --host ${2:-"localhost"} --eval "${js_command}"; then
...
function setUpAAA(){
  ...
  local js_command="db.getSiblingDB('${MONGODB_FHAAA_DATABASE}').addUser({user: '${MONGODB_FHAAA_USER}', pwd: '${MONGODB_FHAAA_PASSWORD}', roles: [ 'readWrite' ]});"
  if ! mongo admin ${1:-} --host ${2:-"localhost"} --eval "${js_command}"; then
...
function setUpSupercore(){
  ...
  local js_command="db.getSiblingDB('${MONGODB_FHSUPERCORE_DATABASE}').addUser({user: '${MONGODB_FHSUPERCORE_USER}', pwd: '${MONGODB_FHSUPERCORE_PASSWORD}', roles: [ 'readWrite' ]});"
  if ! mongo admin ${1:-} --host ${2:-"localhost"} --eval "${js_command}"; then
